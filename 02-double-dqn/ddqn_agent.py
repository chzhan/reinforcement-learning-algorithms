...
class ddqn_agent:
    ...
    def _update_network(self, mini_batch):
        ...
        _, _, action_max = self.deep_q_network(state_next_batch_tensor)
        action_max = action_max.unsqueeze(1)
        action_max = action_max.detach()
        with torch.no_grad():
            target_q_value, _, _ = self.target_network(state_next_batch_tensor)
        target_q_value = target_q_value.gather(1, action_max)
        target_q_value = target_q_value.detach()
        # get the target value...
        target = reward_batch_tensor + self.args.gamma * target_q_value * done_batch_tensor
